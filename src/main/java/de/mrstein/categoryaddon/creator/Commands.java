...
public class Commands implements CommandExecutor {
    ...
    private String[] rA = new String[] {"Nah", "Consoles dont have Inventories", "Yes", "Hm", "I have no idea how I got here", "Example Text", "yay", "TODO: Add something here lol"};
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if(command.getName().equalsIgnoreCase("categoryeditor") || command.getName().equalsIgnoreCase("cedit")) {
            if(player.hasPermission("cc.editor.open")) {
                if (args.length > 0) {
                    if(args[0].contains(":")) {
                        SubCategory category = CustomHeads.getCategoryLoader().getSubCategory(args[0]);
                        if(category != null) {
                        ...
                        Category category = CustomHeads.getCategoryLoader().getCategory(args[0]);
                        if(category != null) {
                            new CategoryEditor(category, player).showCategoryInfos();
                            return true;
                        }
